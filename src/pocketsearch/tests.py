...
class SQLFunctionTests(BaseTest):
    def test_highlight(self):
        self.assertEqual(
            "*fox*"
            in self.pocket_search.search(text="fox").highlight("text")[0].text,
            True,
        )
    ...
    def test_highlight_alternative_marker(self):
        self.assertEqual(
            "<b>fox</b>"
            in self.pocket_search.search(text="fox")
            .highlight("text", marker_start="<b>", marker_end="</b>")[0]
            .text,
            True,
        )
    ...
    def test_snippet(self):
        ...
        systems,[3] used on a large scale for example in search engines.
        """
        self.pocket_search.insert(text=text)
        result = (
            self.pocket_search.search(text="forward index")
            .snippet("text")[0]
            .text
        )
        eq = self.assertEqual
        ...
        result = (
            self.pocket_search.search(text="forward index")
            .snippet("text", text_before="<b>", text_after="</b>")[0]
            .text
        )
...
class OperatorSearch(BaseTest):
    ...
    def test_search_prefix_explicit(self):
        # by default, prefix search is not supported:
        self.assertEqual(
            self.pocket_search.search(text__allow_prefix="fran*").count(), 1
        )
    ...
    def test_initial_token_queries_explicit(self):
        ...
        self.assertEqual(
            self.pocket_search.search(
                text__allow_initial_token="^england"
            ).count(),
            1,
        )
        self.assertEqual(
            self.pocket_search.search(
                text__allow_initial_token__allow_prefix="^engl*"
            ).count(),
            1,
        )
    ...
    def test_search_and_or_query_default(self):
        ...
        self.assertEqual(
            self.pocket_search.search(text="france AND paris").count(), 0
        )
        self.assertEqual(
            self.pocket_search.search(text="france OR paris").count(), 0
        )

    def test_near_query(self):
        # NEAR queries are not supported at the moment
        self.assertEqual(
            self.pocket_search.search(text="NEAR(one, two)").count(), 0
        )

    def test_combined_lookups(self):
        self.assertEqual(
            self.pocket_search.search(
                text__allow_boolean__allow_prefix="france OR engl*"
            ).count(),
            2,
        )
    ...
    def test_search_and_or_query_explicit(self):
        ...
        self.assertEqual(
            self.pocket_search.search(
                text__allow_boolean="france AND paris"
            ).count(),
            1,
        )
        self.assertEqual(
            self.pocket_search.search(
                text__allow_boolean="france OR paris"
            ).count(),
            1,
        )
        self.assertEqual(
            self.pocket_search.search(
                text__allow_boolean="france OR england OR fence"
            ).count(),
            3,
        )
    ...
    def test_negation_default(self):
        ...
        self.assertEqual(
            self.pocket_search.search(text="NOT france").count(), 0
        )
...
class PhraseSearch(unittest.TestCase):
    def test_search_phrase(self):
        ...
        self.assertEqual(
            pocket_search.search(text="This is a phrase").count(), 2
        )
        self.assertEqual(
            pocket_search.search(text="this phrase a is").count(), 2
        )
        self.assertEqual(
            pocket_search.search(text='"this is a phrase"').count(), 1
        )
        pocket_search.close()
    ...
    def test_multiple_phrases(self):
        ...
        self.assertEqual(
            pocket_search.search(text='"this is" "phrase a"').count(), 0
        )
        pocket_search.close()
...
class ContextManagerTest(unittest.TestCase):
    def test_context_manager(self):
        ...
        with PocketReader() as pocketsearch:
            self.assertEqual(
                pocketsearch.search(text="Hello world.").count(), 0
            )
...
class IndexTest(BaseTest):
    ...
    def test_use_insert_update_with_lookups(self):
        ...
        with self.assertRaises(pocket_search.FieldError):
            pocket_search.update(
                rowid=1,
                text__allow_boolean="The DOG jumped over the fence. Now he is beyond the fence.",
            )
        pocket_search.close()
    ...
    def test_count(self):
        ...
        self.assertEqual(
            self.pocket_search.search(text="notinindex").count(), 0
        )

    def test_indexing(self):
        self.assertEqual(
            self.pocket_search.search(text="fence")[0].text, self.data[0]
        )
        self.assertEqual(
            self.pocket_search.search(text="is")[0].text, self.data[1]
        )
...
class QTests(unittest.TestCase):
    ...
    def test_deprecated_union_search(self):
        ...
        self.pocketsearch.search(product="apple") | self.pocketsearch.search(
            product="peach"
        )
    ...
    def test_initial_token_query(self):
        ...
        q = self.pocketsearch.search(
            Q(price__gte=3) & Q(product__allow_initial_token="^Orange")
        )
        self.assertEqual(q.count(), 1)

    def test_and(self):
        q = self.pocketsearch.search(
            Q(product="Apple") & Q(code__allow_prefix="A*") & Q(price__gte=2)
        )
        self.assertEqual(q.count(), 1)
        # Test the other way around:
        q = self.pocketsearch.search(
            Q(price__gte=2) & Q(product="Apple") & Q(code__allow_prefix="A*")
        )
        self.assertEqual(q.count(), 1)
        # Mixed
        q = self.pocketsearch.search(
    ...
    def test_combined_and_or(self):
        q = self.pocketsearch.search(
            Q(price__gte=1) & Q(price__lte=5) & Q(product="Apple")
            | Q(product="Orange")
        )
        self.assertEqual(q.count(), 2)
...
class IndexUpdateTests(BaseTest):
    ...
    def test_update_entry(self):
        ...
        self.pocket_search.update(
            rowid=1,
            text="The DOG jumped over the fence. Now he is beyond the fence.",
        )
        self.assertEqual(self.pocket_search.search(text="fox").count(), 0)
...
class EscapeTests(unittest.TestCase):
    def test_escape_characters(self):
        ...
        # in this case, punctuation characters are automatically removed from
        # the query:
...
class AutocompleteTest(unittest.TestCase):
    ...
    def test_autocomplete_one_token(self):
        ...
        """
        self.assertEqual(
            self.pocket_search.autocomplete(text="In")[0].text, "Indiana Jones"
        )
        self.assertEqual(
            self.pocket_search.autocomplete(text="In")[1].text, "Jones Indiana"
        )
        self.assertEqual(
            self.pocket_search.autocomplete(text="Ind")[0].text,
            "Indiana Jones",
        )
        self.assertEqual(
            self.pocket_search.autocomplete(text="In")[1].text, "Jones Indiana"
    ...
    def test_order_by(self):
        ...
        """
        self.assertEqual(
            self.pocket_search.autocomplete(text="In")
            .order_by("-rank")[0]
            .text,
            "Jones Indiana",
        )
    ...
    def test_autocomplete_two_tokens(self):
        ...
        """
        self.assertEqual(
            self.pocket_search.autocomplete(text="Indiana J")[0].text,
            "Indiana Jones",
        )
    ...
    def test_autocomplete_including_and_or_operators(self):
        ...
        """
        self.assertEqual(
            self.pocket_search.autocomplete(text="INDIANA OR JONES").count(), 0
        )
        self.assertEqual(
            self.pocket_search.autocomplete(text="INDIANA AND JONES").count(),
            0,
        )
    ...
    def test_autocomplete_three_tokens(self):
        ...
        """
        self.assertEqual(
            self.pocket_search.autocomplete(text="return of the")[0].text,
            "The return of the Jedi",
        )
...
class Unicode61Tests(unittest.TestCase):
    ...
    def test_tokenize_custom_categories_and_separators(self):
        ...
        tokens = Unicode61(categories="N*", separators="4").tokenize(
            under_test
        )
...
class TokenizerTests(unittest.TestCase):
    ...
    def test_mix_categories_and_separators(self):
        ...
        # This results in only one token as everything else is considered a
        # separator
    ...
    def test_token_chars(self):
        ...
        pocket_search.insert(text=under_test)
        self.assertEqual(
            pocket_search.search(text="tokenization@test.test").count(), 1
        )
...
class CharacterTest(unittest.TestCase):
    ...
    def test_search_hyphen(self):
        ...
        """
        self.assertEqual(
            self.pocket_search.search(text="break even").count(), 1
        )
        self.assertEqual(
            self.pocket_search.search(text="break-even").count(), 1
        )
        self.assertEqual(
            self.pocket_search.search(text="breakeven").count(), 0
        )
    ...
    def test_search_punctuation(self):
        ...
        """
        self.assertEqual(
            self.pocket_search.search(text__allow_prefix="I50.*").count(), 2
        )
    ...
    def test_pocket_reader_normalize(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            with PocketWriter(
                db_name=os.path.join(tmpdirname, "index.db"),
                normalize=normalize,
            ) as writer:
                writer.insert(text="U.S.A.")
            with PocketReader(
...
class MultipleFieldRankingtest(unittest.TestCase):
    ...
    def test_rank_multiple_fields_and_query(self):
        self.assertEqual(
            self.pocket_search.search(text="A", title="A", category="A")[
                0
            ].title,
            "A",
        )
...
class StemmingTests(unittest.TestCase):
    ...
    def test_search(self):
        ...
        self.assertEqual(
            self.pocket_search.search(text="anforderung").count(), 0
        )
...
class MultipleFieldIndexTest(unittest.TestCase):
    def setUp(self):
        ...
        self.data = [
            (
                "Blade Runner",
                (
                    "Blade Runner [bleɪd ˌrʌnɚ], deutscher Verleihtitel zeitweise "
                    "auch Der Blade Runner, ist ein am 25. Juni 1982 erschienener "
                    "US-amerikanischer Science-Fiction-Film des Regisseurs "
                    "Ridley Scott."
                ),
            ),
        ]
    ...
    def test_all_fields_available_in_results(self):
        self.assertEqual(
            self.pocket_search.search(text="ˌrʌnɚ")[0].title, "Blade Runner"
        )

    def test_set_values_in_results(self):
        self.assertEqual(
            self.pocket_search.search(text="ˌrʌnɚ").values("title")[0].title,
            "Blade Runner",
        )
    ...
    def test_set_non_existing_field_in_results(self):
        with self.assertRaises(Schema.SchemaError):
            self.assertEqual(
                self.pocket_search.search(text="ˌrʌnɚ")
                .values("title323232")[0]
                .title,
                "Blade Runner",
            )
    ...
    def test_combined_field_search(self):
        self.assertEqual(
            self.pocket_search.search(text="Blade", title="runner").count(), 1
        )
...
class FieldTypeTests(unittest.TestCase):
    ...
    def test_search_dates_range(self):
        ...
        self.assertEqual(
            self.pocket_search.search(
                f6__year__gte=year - 1, f6__year__lte=year + 1
            ).count(),
            1,
        )
...
class StructuredDataTests(unittest.TestCase):
    ...
    def test_search_category(self):
        ...
        self.assertEqual(
            self.pocket_search.search(category="Fruit").count(), 4
        )

    def test_search_category_operators(self):
        # Should work, as sqlite3 allows this:
        self.assertEqual(
            self.pocket_search.search(category__lte="Fruit").count(), 4
        )

    def test_filter_combined(self):
        self.assertEqual(
            self.pocket_search.search(
                price__lte=3, description="apple"
            ).count(),
            1,
        )
    ...
    def test_price_range(self):
        self.assertEqual(
            self.pocket_search.search(price__lt=6, price__gt=4).count(), 1
        )

    def test_filter_combined_and_or(self):
        self.assertEqual(
            self.pocket_search.search(
                price__lte=4, description__allow_boolean="apple OR Orange"
            ).count(),
            2,
        )
    ...
    def test_build_index(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            for file_name, contents in [
                ...
                f = open(
                    os.path.join(tmpdirname, file_name), "w", encoding="utf-8"
                )
                f.write(contents)
                f.close()
...
class SpellCheckerTest(unittest.TestCase):
    ...
    def test_suggest(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with PocketWriter(
                db_name=self.db_name, schema=self.TestSchema
            ) as pocketwriter:
                for title, text in [
    ...
    def test_change_schema(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with PocketReader(db_name=db_name, schema=self.Article) as reader:
                with PocketWriter(
                    db_name=db_name,
                    index_name="document_v2",
                    schema=self.NewArticle,
                ) as writer:
            ...
            with PocketReader(
                ...
                self.assertEqual(
                    reader.search(title="some default").count(), 2
                )
    ...
    def test_no_id_legacy_table(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(PocketSearch.DatabaseError):
                PocketSearch(
                    index_name="no_id_field",
                    db_name=db_name,
                    schema=self.LegacyTableSchemaMissingField,
                    writeable=True,
                )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(0, self.NUM_THREADS):
                thread = threading.Thread(
                    ...
                    args=("Writing data from Thread %i " % i, db_name),
                )
            ...
            self.assertEqual(
                p.search().count(), self.NUM_THREADS * self.NUM_INSERTS
            )
