...
class ConnectionPool:
    ...
    def get_connection(self, db_name, writeable, conn_id=None):
        ...
        if writeable:
            ...
            if not self.connections[conn_id]["writer"].acquire(
                timeout=self.POOL_CONNECTION_TIMEOUT
            ):
                raise self.ConnectionError(
                    f"Unable to acquire pocketsearch writer "
                    f"(Timed out) for {conn_id}"
                )
...
class SpellChecker:
    ...
    def __init__(self, search_instance):
        ...
        self.spell_checker = PocketSearch(
            db_name=search_instance.db_name,
    ...
    def suggest(self, query):
        ...
        for cleaned_token in set(Unicode61().tokenize(query)):
            if len(cleaned_token) > 1:
                ...
                for result in self.spell_checker.search(
                    bigrams__allow_boolean=self._generate_bigrams(
                        cleaned_token, " OR "
                    )
                )[0:10]:
...
class PocketSearch:
    ...
    class DocumentDoesNotExist(Exception):
        """
        Thrown when accessing a document through .get that does not exist.
        """
    ...
    class Argument:
        """
        Helper class storing fields and lookups from keyword arguments.
        """
    ...
    def __init__(
        ...
        if self.normalize is not None and not (
            isinstance(self.normalize, types.FunctionType)
        ):
    ...
    def spell_checker(self):
        ...
        if self.spell_checker is None:
            raise self.schema.SchemaError(
                "PocketSearch instance is not configured to use spell "
                "checking. Check your schema definition."
            )
    ...
    def _open(self):
        return connection_pool.get_connection(
            db_name=self.db_name, writeable=self.writeable, conn_id=self.db_id
        )
    ...
    def assure_writeable(self):
        ...
        if not self.writeable:
            raise self.IndexError(
                "Index '{schema_name}' has been opened in read-only mode. "
                "Cannot write changes to index.".format(
    ...
    def execute_sql(self, sql, *args):
        ...
        sql contains the query, *args the arguments.
        """
    ...
    def _format_sql(self, index_name, fields, sql):
        ...
        return sql.format(
            ...
            new_cols=", ".join(
                [
                    "new.%s" % field.to_sql(index_table=True)
                    for field in fields
                    if field.fts_enabled()
                ]
            ),
            old_cols=", ".join(
                [
                    "old.%s" % field.to_sql(index_table=True)
                    for field in fields
                    if field.fts_enabled()
                ]
            ),
    ...
    def _create_additional_options(self):
        """
        Reads schema Meta options for FTS table creation.

        Specifically configures the tokenization process.
        """
        m = self.schema._meta
    ...
    def _create_prefix_index(self):
        ...
        if prefix_index is not None:
            ...
            if not all(
                isinstance(item, int) and item > 0 for item in prefix_index
            ):
                raise self.schema.SchemaError(
                    "prefix_index list should only contain positive "
                    "integer values."
                )
            # eliminate duplicates
            prefix_index = set(prefix_index)
            return ", prefix='{prefix_index}'".format(
                prefix_index=" ".join(str(item) for item in prefix_index)
            )
        return ""
    ...
    def _table_exists(self):
        ...
        self.cursor.execute(
            "SELECT * from pocket_search_master where name = ?",
            (self.index_name,),
        )
    ...
    def _check_fields(self):
        ...
        fields = {}
        mappings = {"INT": "INTEGER", "FLOAT": "REAL"}
            ...
            if field != "rank":
                if field not in fields:
                    raise self.DatabaseError(
                        f"'{field}' is present in the schema but has not "
                        f"been defined in the legacy table."
                    )
                if definition.data_type != fields[field]:
                    legacy_definition = fields[field]
                    raise self.DatabaseError(
                        f"'{field}' has data type '{definition.data_type}' "
                        f"in schema but '{legacy_definition}' was expected."
                    )
        return True
    ...
    def _create_table(self, index_name):
        ...
        # Create meta table holding all pocketsearch created search index
        # names:
        self.cursor.execute(
            "CREATE TABLE IF NOT EXISTS pocket_search_master "
            "(name TEXT unique, content TEXT)"
        )
        # Check if the table is already there and if it is under management of
        # pocketsearch:
        ...
        sql_table = (
            f"CREATE TABLE IF NOT EXISTS {index_name}({standard_fields})"
        )
        sql_virtual_table = f"""
        ...
        # aux tables
        sql_aux_table = (
            f"CREATE VIRTUAL TABLE IF NOT EXISTS {index_name}_fts_v "
            f"USING fts5vocab('{index_name}_fts', 'row');"
        )
        # Trigger definitions:
        old_cols = ", ".join(
            [
        ...
        INSERT INTO {index_name}_fts({index_name}_fts, rowid, {fts_fields})
        VALUES('delete', old.{id_field}, {old_cols});
        END;"""
        sql_trigger_update = f"""
        CREATE TRIGGER IF NOT EXISTS {index_name}_au AFTER UPDATE ON {index_name} BEGIN
        INSERT INTO {index_name}_fts({index_name}_fts, rowid, {fts_fields})
        VALUES('delete', old.{id_field}, {old_cols});
        INSERT INTO {index_name}_fts(rowid, {fts_fields})
        VALUES (new.{id_field}, {new_cols});
        END;
        """
        ...
        self.cursor.execute(
            self._format_sql(index_name, fields, sql_trigger_insert)
        )
        self.cursor.execute(
            self._format_sql(index_name, fields, sql_trigger_delete)
        )
        self.cursor.execute(
            self._format_sql(index_name, fields, sql_trigger_update)
        )
        if table_exists:
            if not managed:
                # Index existing data, this will be executed only once
                sql_index_data = (
                    f"INSERT INTO {index_name}_fts (rowid, {fts_fields}) "
                    f"SELECT ROWID, {fts_fields} FROM {index_name}"
                )
                logger.debug(sql_index_data)
                self.cursor.execute(sql_index_data)
                self.cursor.execute(
                    "insert or ignore into pocket_search_master values (?,?)",
                    (index_name, content),
                )
        else:
            ...
            for field in default_index_fields:
                self.cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_std_{index_name}_{field} "
                    "ON {index_name} ({field});".format(
    ...
    def tokens(self, top_n=25):
        ...
        """
        sql = (
            f"select term as token, doc as num_documents, "
            f"cnt as total_count from {self.index_name}_fts_v "
            f"order by total_count desc LIMIT {top_n}"
        )
    ...
    def get_arguments(self, kwargs, for_search=True, normalize=None):
        """
        Extract field names and lookups from keyword arguments.

        Returns a dictionary of argument objects.
        """
        referenced_fields = {}
        id_field = self.schema.get_id_field() or "id"
        for kwarg in kwargs:
            ...
            for lookup in lookups:
                for name in lookup.names:
                    if not name in LOOKUPS:
                        raise self.FieldError(
                            f"""Unknown lookup: '{name}' in field '{f}'"""
                        )
        arguments = {}
        for field in self.schema:
            if (
                field.name not in referenced_fields
                and not (for_search)
                and field.name not in [id_field, "rank"]
            ):
                raise self.FieldError(
    ...
    def insert_or_update(self, **kwargs):
        ...
        if self.schema.id_field is None:
            raise self.DatabaseError(
                "No IDField has been defined in the schema - "
    ...
    def update(self, **kwargs):
        ...
        values = [
            argument.lookups[0].value for argument in arguments.values()
        ] + [docid]
        stmt = []
        for f in arguments:
            stmt.append("%s=?" % f)
    ...
    def autocomplete(self, *args, **kwargs):
        ...
        if len(args) > 0:
            raise Query.QueryError(
                """.autocomplete expects exactly one keyword argument
        ...
        if len(query_components) > 1:
            ...
            prefix = "*"
        query_components[0] = (
            "(^{first_word}{prefix} OR {first_word}{prefix})".format(
                first_word=query_components[0], prefix=prefix
            )
        )
        if len(query_components) > 1:
            query_components[len(query_components) - 1] = (
                query_components[-1:][0] + "*"
            )
        query = " AND ".join(query_components)
        return self.search(
            **{
                f"{field}__allow_boolean__allow_prefix__allow_initial_token": query
            }
        )
    ...
    def search(self, *args, **kwargs):
        ...
        if len(args) > 0:
            ...
            return Query(
                search_instance=self, arguments=[], q_arguments=args[0]
            )
        arguments = self.get_arguments(
