...
class Tokenizer(abc.ABC):
    ...
    def to_sql(self):
        properties = " ".join(
            ["%s '%s'" % (p.name, p.value) for p in self.properties.values()]
        )
        return 'tokenize="{name} {properties}"'.format(
            name=self.name, properties=properties
        )
...
class Unicode61(Tokenizer):
    ...
    def __init__(
        ...
        if (
            remove_diacritics not in self.VALID_DIACRITICS
            and remove_diacritics is not None
        ):
            raise self.TokenizerError(
                "Invalid valid for remove_diacritics. Valid options are %s"
                % self.VALID_DIACRITICS
            )
