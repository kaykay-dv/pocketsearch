...
def convert_timestamp(value):
    ...
    return datetime.datetime.strptime(
        value.decode("utf-8").split(".")[0], "%Y-%m-%d %H:%M:%S"
    )
...
class Timer:
    ...
    def get_its(self):
        ...
        """
        return round(
            1 / ((self.stop - self.start) / self.snapshots), self.precision
        )
    ...
    def snapshot(self, more_info=""):
        ...
        its = self.get_its()
        out = "%s iterations %s it/s %s s elapsed %s%s" % (
            self.snapshots,
            its,
            round(self.stop - self.start, self.precision),
            more_info,
            " " * 15,
        )
    ...
    def __str__(self):
        ...
        for lap_name, lap_time in self.laps:
            lap_name_display = (
                lap_name + (len(longest_string) - len(lap_name)) * " "
            )
            s = s + "%s %s s\n" % (
                lap_name_display,
